...
import generatelighthouseReadme from '../utils/lighthouse_commit';
...
export class GithubWorkerQueue {
    ...
    private async process_job(job: Job<GithubPushJobData>) {
        ...
        try {
            ...
            if (created && !files.some((f) => f.path === 'README.md')) {
                files = [...files, { path: 'README.md', content: generatelighthouseReadme() }];
            }
    ...
    private async upsert_code(octokit: Octokit, owner: string, repo: string, files: FileContent[]) {
        ...
        const commit = await octokit.git.createCommit({
            owner,
            repo,
            message: 'lighthouse deployment',
            tree: new_tree.data.sha,
            parents: [base_sha],
            author: { name: 'lighthouse', email: 'lighthouse.dev.official@gmail.com' },
            committer: { name: 'lighthouse', email: 'lighthouse.dev.official@gmail.com' },
        });
